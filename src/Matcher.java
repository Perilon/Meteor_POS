...
public class Matcher {
	public static void main(String[] args, MaxentTagger tagger) throws Exception {
		...
		URL AdjadvURL = Constants.getDefaultPoscatsAdjadvUrl(Constants.getLanguageID(language));
		URL NounURL = Constants.getDefaultPoscatsNounUrl(Constants.getLanguageID(language));
		URL OtherURL = Constants.getDefaultPoscatsOtherUrl(Constants.getLanguageID(language));
		URL VerbURL = Constants.getDefaultPoscatsVerbUrl(Constants.getLanguageID(language));
		...
		ArrayList<Double> newDeltaWeights = new ArrayList<Double>();
		...
		// Construct aligner
		Aligner aligner = new Aligner(tagger, language, modules, moduleWeights, newDeltaWeights,
				beamSize, Constants.getDefaultWordFileURL(Constants
						.getLanguageID(language)), synURL, paraURL, AdjadvURL, NounURL, OtherURL, VerbURL,
				partialComparator);
		...
		if (stdio) {
			...
			while ((lineTest = inTest.readLine()) != null) {
				...
				try {
					// Align
					Alignment a = aligner.align(lineTest, lineRef, tagger);
					// Output results
