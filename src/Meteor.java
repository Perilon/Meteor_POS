...
public class Meteor {
	...
	public static void main(String[] args) {
		...
		if (!ssOut && !stdio && !quiet) {
			...
			System.out.println("New d. weights: "
					+ config.getNewDeltaWeightsString());
			System.out.println();
		}
		...
		if (config.getNewDeltaWeights().size() != 4) {
			System.err.println("Warning: You have not specified exactly 4 new delta weights. "
					+ "If you are attempting to use new delta weights, "
					+ "there must be exactly 4 specified.");
		}
	...
	private static void scorePlaintext(MeteorScorer scorer, Properties props,
			MeteorConfiguration config, String testFile, String refFile, MaxentTagger tagger)
			throws IOException {
		...
		for (int i = 0; i < lines1.size(); i++) {
			MeteorStats stats;
			if (refCount == 1) {
				stats = scorer.getMeteorStats(lines1.get(i), lines2.get(i), tagger);
			} else
				stats = scorer.getMeteorStats(lines1.get(i), lines2mref.get(i), tagger);
			if (ssOut) {
				...
				System.out.println("Segment " + (i + 1) + " Xscore:\t"
						+ stats.Xprecision + "\t" + stats.Xrecall + "\t"
						+ stats.fragPenalty + "\t" + stats.Xscore);
			} else if (quiet) {
				System.err.println(stats.score);
				System.err.println(stats.Xscore);
			} else {
	...
	private static void scoreStdio(MeteorScorer scorer, MaxentTagger tagger) throws IOException {
		...
		while ((line = in.readLine()) != null) {
			...
			if (line.startsWith("SCORE") || line.startsWith("score")) {
				...
				MeteorStats stats = scorer.getMeteorStats(test, refs, tagger);
				System.out.println(stats.toString());
	...
	private static void printVerboseStats(MeteorStats stats,
		...
		System.out
				.println("           Test Matches                              " +
						"         Reference Matches");
		System.out
				.println("Stage      Adjadvs     Nouns    Other      Verbs     Total    " +
						"Adjadvs     Nouns     Other     Verbs     Total");
		double XtestAdjadvMatches = 0.0;
		double XtestNounMatches = 0.0;
		double XtestOtherMatches = 0.0;
		double XtestVerbMatches = 0.0;
		double XreferenceAdjadvMatches = 0.0;
		double XreferenceNounMatches = 0.0;
		double XreferenceOtherMatches = 0.0;
		double XreferenceVerbMatches = 0.0;

		for (int i = 0; i < weights.size(); i++) {
			System.out
					.println(String.format(
							"%1d          %7s   %7s  %7s    %7s   %7s  %9s   %7s   %7s   %7s   %7s",
							i + 1,
							Constants.minFormat.format(stats.XtestStageMatches_Adjadvs
									.get(i)),
							Constants.minFormat.format(stats.XtestStageMatches_Nouns
									.get(i)),
							Constants.minFormat.format(stats.XtestStageMatches_Other
									.get(i)),
							Constants.minFormat.format(stats.XtestStageMatches_Verbs
									.get(i)),
							Constants.minFormat.format(stats.XtestStageMatches_Adjadvs.get(i)
									+ stats.XtestStageMatches_Nouns.get(i)
									+ stats.XtestStageMatches_Other.get(i)
									+ stats.XtestStageMatches_Verbs.get(i)),
							Constants.minFormat.format(stats.XreferenceStageMatches_Adjadvs
									.get(i)),
							Constants.minFormat.format(stats.XreferenceStageMatches_Nouns
									.get(i)),
							Constants.minFormat.format(stats.XreferenceStageMatches_Other
									.get(i)),
							Constants.minFormat.format(stats.XreferenceStageMatches_Verbs
									.get(i)),
							Constants.minFormat.format(stats.XreferenceStageMatches_Adjadvs.get(i)
									+ stats.XreferenceStageMatches_Nouns.get(i)
									+ stats.XreferenceStageMatches_Other.get(i)
									+ stats.XreferenceStageMatches_Verbs.get(i))));
			XtestAdjadvMatches += stats.XtestStageMatches_Adjadvs.get(i);
			XtestNounMatches += stats.XtestStageMatches_Nouns.get(i);
			XtestOtherMatches += stats.XtestStageMatches_Other.get(i);
			XtestVerbMatches += stats.XtestStageMatches_Verbs.get(i);
			XreferenceAdjadvMatches += stats.XreferenceStageMatches_Adjadvs.get(i);
			XreferenceNounMatches += stats.XreferenceStageMatches_Nouns.get(i);
			XreferenceOtherMatches += stats.XreferenceStageMatches_Other.get(i);
			XreferenceVerbMatches += stats.XreferenceStageMatches_Verbs.get(i);
		}
		System.out.println(String.format(
				"Total      %7s   %7s  %7s    %7s   %7s  %9s   %7s   %7s   %7s   %7s",
				Constants.minFormat.format(XtestAdjadvMatches),
				Constants.minFormat.format(XtestNounMatches),
				Constants.minFormat.format(XtestOtherMatches),
				Constants.minFormat.format(XtestVerbMatches),
				Constants.minFormat.format(XtestAdjadvMatches + XtestNounMatches
						+ XtestOtherMatches + XtestVerbMatches),
				Constants.minFormat.format(XreferenceAdjadvMatches),
				Constants.minFormat.format(XreferenceNounMatches),
				Constants.minFormat.format(XreferenceOtherMatches),
				Constants.minFormat.format(XreferenceVerbMatches),
				Constants.minFormat.format(XreferenceAdjadvMatches + XreferenceNounMatches
						+ XreferenceOtherMatches + XreferenceVerbMatches)));
		System.out.println();
		...
		System.out.println("Xprecision:             " + stats.Xprecision);
		System.out.println("Xrecall:                " + stats.Xrecall);
		System.out.println("Xf1:                    " + stats.Xf1);
		System.out.println("XfMean:                 " + stats.XfMean);
	...
	public static Properties createPropertiesFromArgs(String[] args,
		...
		while (curArg < args.length) {
			...
			// New!  Set new delta parameters

			} else if (args[curArg].equals("-ndw")) {
				props.setProperty("newDeltaWeights", args[curArg + 1]);
				curArg += 2;
