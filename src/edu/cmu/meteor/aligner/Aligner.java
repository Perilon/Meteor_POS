...
public class Aligner {
	...
	private HashSet<String> Adjadvs;
	...
	public Aligner(MaxentTagger tagger, String language, ArrayList<Integer> modules) {
		...
		setupModules(language, modules, Constants.DEFAULT_WORD_DIR_URL,
				...
				Constants.getDefaultPoscatsAdjadvUrl(Constants
						.getLanguageID(Constants.normLanguageName(language))),

				Constants.getDefaultPoscatsNounUrl(Constants.
						getLanguageID(Constants.normLanguageName(language))),

				Constants.getDefaultPoscatsOtherUrl(Constants.
						getLanguageID(Constants.normLanguageName(language))),

				Constants.getDefaultPoscatsVerbUrl(Constants.
						getLanguageID(Constants.normLanguageName(language)))		);
	}
	...
	public Aligner(MaxentTagger tagger, String language, ArrayList<Integer> modules,
			ArrayList<Double> moduleWeights, ArrayList<Double> newDeltaWeights,
			int beamSize, URL wordFileURL,
			URL synDirURL, URL paraDirURL, URL poscatsAdjadvURL, URL poscatsNounURL,
			URL poscatsOtherURL, URL poscatsVerbURL) {
		this.beamSize = beamSize;
		this.partialComparator = Constants.PARTIAL_COMPARE_TOTAL;

		setupModules(language, modules, wordFileURL, synDirURL, paraDirURL, poscatsAdjadvURL, poscatsNounURL,
				poscatsOtherURL, poscatsVerbURL);
	...
	public Aligner(Aligner aligner) {
		...
		// New!  New delta weights
		this.newDeltaWeights = new ArrayList<Double>(aligner.newDeltaWeights);
		...
		this.Adjadvs = aligner.Adjadvs;
		this.Nouns = aligner.Nouns;
		this.Other = aligner.Other;
		this.Verbs = aligner.Verbs;
	}

	private void setupModules(String language, ArrayList<Integer> modules,
			URL wordFileURL, URL synDirURL, URL paraDirURL, URL poscatsAdjadvURL, URL poscatsNounURL,
			URL poscatsOtherURL, URL poscatsVerbURL) {
		...
		for (int i = 0; i < this.modules.size(); i++) {
			...
			if (module == Constants.MODULE_EXACT) {
				...
				try {
					URL excFileURL = new URL(synDirURL.toString() + "/" + this.language + ".exceptions");
					URL synFileURL = new URL(synDirURL.toString() + "/" + this.language + ".synsets");
					URL relFileURL = new URL(synDirURL.toString() + "/" + this.language + ".relations");
		...
		this.Adjadvs = new HashSet<String>();
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(
					poscatsAdjadvURL.openStream(), "UTF-8"));
			String line;
			while ((line = in.readLine()) != null) {
				this.Adjadvs.add(line);
			}
			in.close();
		} catch (IOException ex) {
			throw new RuntimeException("No Poscats Adjadvs list ("
					+ poscatsAdjadvURL.toString() + ")");
		}

		this.Nouns = new HashSet<String>();
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(
					poscatsNounURL.openStream(), "UTF-8"));
			String line;
			while ((line = in.readLine()) != null) {
				this.Nouns.add(line);
			}
			...
			throw new RuntimeException("No Poscats Nouns list ("
					+ poscatsAdjadvURL.toString() + ")");
		}
		...
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(
					poscatsOtherURL.openStream(), "UTF-8"));
			String line;
			while ((line = in.readLine()) != null) {
				this.Other.add(line);
			}
			...
			throw new RuntimeException("No Poscats Other list ("
					+ poscatsOtherURL.toString() + ")");
		}

		this.Verbs = new HashSet<String>();
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(
					poscatsVerbURL.openStream(), "UTF-8"));
			String line;
			while ((line = in.readLine()) != null) {
				this.Verbs.add(line);
			}
			...
			throw new RuntimeException("No Poscats Verbs list ("
					+ poscatsVerbURL.toString() + ")");
		}
	}
	...
	// New!  New delta weights
	public void updateNewDeltaWeights(ArrayList<Double> newDeltaWeights) {
		this.newDeltaWeights = new ArrayList<Double>(newDeltaWeights);
	}

	public Alignment align(String line1, String line2, MaxentTagger tagger) {
		Alignment a = new Alignment(line1, line2, tagger);
		align(a);
		return a;
	}
	...
	private void align(Alignment a) {
		...
		int[] adjadvMatches1 = new int[moduleCount];
		int[] adjadvMatches2 = new int[moduleCount];
		Arrays.fill(adjadvMatches1, 0);
		Arrays.fill(adjadvMatches2, 0);
		int[] nounMatches1 = new int[moduleCount];
		int[] nounMatches2 = new int[moduleCount];
		Arrays.fill(nounMatches1, 0);
		Arrays.fill(nounMatches2, 0);
		int[] otherMatches1 = new int[moduleCount];
		int[] otherMatches2 = new int[moduleCount];
		Arrays.fill(otherMatches1, 0);
		Arrays.fill(otherMatches2, 0);
		int[] verbMatches1 = new int[moduleCount];
		int[] verbMatches2 = new int[moduleCount];
		Arrays.fill(verbMatches1, 0);
		Arrays.fill(verbMatches2, 0);
		...
		boolean[] isAdjadv1 = new boolean[a.tags1.size()];
		boolean[] isAdjadv2 = new boolean[a.tags2.size()];
		boolean[] isNoun1 = new boolean[a.tags1.size()];
		boolean[] isNoun2 = new boolean[a.tags2.size()];
		boolean[] isOther1 = new boolean[a.tags1.size()];
		boolean[] isOther2 = new boolean[a.tags2.size()];
		boolean[] isVerb1 = new boolean[a.tags1.size()];
		boolean[] isVerb2 = new boolean[a.tags2.size()];
		...
		// Check for AdjAdvs, Nouns, Verbs, Other
		for (int i = 0; i < a.tags1.size(); i++)
			if (Adjadvs.contains(a.tags1.get(i))) {
				isAdjadv1[i] = true;
				a.line1Adjadvs.add(i);
			}
			else if (Nouns.contains(a.tags1.get(i))) {
				isNoun1[i] = true;
				a.line1Nouns.add(i);
			}
			else if (Verbs.contains(a.tags1.get(i))) {
				isVerb1[i] = true;
				a.line1Verbs.add(i);
			}
			else {
...
//			else if (Other.contains(a.tags1.get(i))) {
//				isOther1[i] = true;
//				a.line1Other.add(i);
//			}

		for (int i = 0; i < a.tags2.size(); i++)
			if (Adjadvs.contains(a.tags2.get(i))) {
				isAdjadv2[i] = true;
				a.line2Adjadvs.add(i);
			}
			else if (Nouns.contains(a.tags2.get(i))) {
				isNoun2[i] = true;
				a.line2Nouns.add(i);
			}
			else if (Verbs.contains(a.tags2.get(i))) {
				isVerb2[i] = true;
				a.line2Verbs.add(i);
			}
			else {
...
//			else if (Other.contains(a.tags2.get(i))) {
//				isOther2[i] = true;
//				a.line2Other.add(i);
//			}
		...
		for (int i = 0; i < best.matches.length; i++) {
			...
			if (m != null) {
				for (int j = 0; j < m.matchLength; j++) {
					...
					if (isAdjadv1[m.matchStart + j])
						adjadvMatches1[m.module]++;
					else if (isNoun1[m.matchStart + j])
						nounMatches1[m.module]++;
					else if (isVerb1[m.matchStart + j])
						verbMatches1[m.module]++;
					else
						otherMatches1[m.module]++;
				}
				for (int j = 0; j < m.length; j++) {
					if (isAdjadv2[m.start + j])
						adjadvMatches2[m.module]++;
					else if (isNoun2[m.start + j])
						nounMatches2[m.module]++;
					else if (isVerb2[m.start + j])
						verbMatches2[m.module]++;
					else
						otherMatches2[m.module]++;
				}
			}
		}
		...
		for (int i = 0; i < moduleCount; i++) {
			...
			a.moduleAdjAdvsMatches1.add(adjadvMatches1[i]);
			a.moduleAdjAdvsMatches2.add(adjadvMatches2[i]);
			a.moduleNounsMatches1.add(nounMatches1[i]);
			a.moduleNounsMatches2.add(nounMatches2[i]);
			a.moduleOtherMatches1.add(otherMatches1[i]);
			a.moduleOtherMatches2.add(otherMatches2[i]);
			a.moduleVerbsMatches1.add(verbMatches1[i]);
			a.moduleVerbsMatches2.add(verbMatches2[i]);
		}
