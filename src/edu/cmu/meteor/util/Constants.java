...
public class Constants {
	...
	public static final URL DEFAULT_POSCATS_ADJADV_DIR_URL = ClassLoader
			.getSystemResource("poscats");

	public static final URL DEFAULT_POSCATS_NOUN_DIR_URL = ClassLoader
			.getSystemResource("poscats");

	public static final URL DEFAULT_POSCATS_OTHER_DIR_URL = ClassLoader
			.getSystemResource("poscats");

	public static final URL DEFAULT_POSCATS_VERB_DIR_URL = ClassLoader
			.getSystemResource("poscats");
	...
	// New delta weights permanent default

	public static final double NDW_DFLT[] = { 0.25, 0.25, 0.25, 0.25 };
	...
	public static URL getDefaultPoscatsAdjadvUrl(int langID) throws RuntimeException {
		String lang = getLanguageName(langID);
		try {
			return new URL(DEFAULT_POSCATS_ADJADV_DIR_URL.toString() + "/" + lang
					+ ".adjadv");
		} catch (MalformedURLException ex) {
			throw new RuntimeException();
		}
	}
	public static URL getDefaultPoscatsNounUrl(int langID) throws RuntimeException {
		String lang = getLanguageName(langID);
		try {
			return new URL(DEFAULT_POSCATS_NOUN_DIR_URL.toString() + "/" + lang
					+ ".noun");
		} catch (MalformedURLException ex) {
			throw new RuntimeException();
		}
	}
	public static URL getDefaultPoscatsOtherUrl(int langID) throws RuntimeException {
		String lang = getLanguageName(langID);
		try {
			return new URL(DEFAULT_POSCATS_OTHER_DIR_URL.toString() + "/" + lang
					+ ".other");
		} catch (MalformedURLException ex) {
			throw new RuntimeException();
		}
	}
	public static URL getDefaultPoscatsVerbUrl(int langID) throws RuntimeException {
		String lang = getLanguageName(langID);
		try {
			return new URL(DEFAULT_POSCATS_VERB_DIR_URL.toString() + "/" + lang
					+ ".verb");
		} catch (MalformedURLException ex) {
			throw new RuntimeException();
		}
...
//	public static ArrayList<Double> getNewDeltaWeights() throws RuntimeException {
//		return getNewDeltaWeights();
//	}

	public static ArrayList<Double> getNewDeltaWeights() {

		// Copy weights
		ArrayList<Double> weights = new ArrayList<Double>();
		for (double weight : NDW_DFLT)
			weights.add(weight);
	...
	public static String getNewDeltaWeightListString(ArrayList<Double> weights) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < weights.size(); i++) {
			sb.append(weights.get(i));
			if (i < weights.size() - 1)
