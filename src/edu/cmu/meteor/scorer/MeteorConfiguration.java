...
public class MeteorConfiguration {
	...
	private void setDefaults() {
		...
		setPoscatsAdjadvURL(Constants.getDefaultPoscatsAdjadvUrl(langID));
		setPoscatsNounURL(Constants.getDefaultPoscatsNounUrl(langID));
		setPoscatsOtherURL(Constants.getDefaultPoscatsOtherUrl(langID));
		setPoscatsVerbURL(Constants.getDefaultPoscatsVerbUrl(langID));
	...
	public String getConfigID() {
		// Format: meteor-version-ch-lang-norm-params-mods-weights-newDeltaWeights
		return "meteor-" + Constants.VERSION + "-"
				...
				+ Constants.getNewDeltaWeightListString(this.newDeltaWeights);
	}
	...
	public void setTask(int taskID) {
		...
		setNewDeltaWeights(Constants.getNewDeltaWeights());

		ArrayList<Double> weightList = Constants.getModuleWeights(langID, taskID);
		...
		// New!  For new delta weights
		ArrayList<Double> newDeltaWeightList = Constants.getNewDeltaWeights();
		ArrayList<Double> newDeltaSizedWeightList = new ArrayList<Double>();
		for (int i = 0; i < newDeltaWeights.size(); i++)
			newDeltaSizedWeightList.add(newDeltaWeightList.get(i));
		setNewDeltaWeights(newDeltaSizedWeightList);
	...
	public ArrayList<Double> getNewDeltaWeights() { return new ArrayList<Double>(newDeltaWeights); }

	public String getNewDeltaWeightsString() {
		StringBuilder sb = new StringBuilder();
		for (Double weight : newDeltaWeights)
			sb.append(weight + " ");
		return sb.toString().trim();
	}

	public void setNewDeltaWeights(ArrayList<Double> deltaScores) {
		setTaskName("custom");
		this.newDeltaWeights = new ArrayList<Double>(deltaScores);
	}
	...
	public void setPoscatsAdjadvURL(URL poscatsAdjadvURL) {
		try {
			// This should not ever throw a malformed url exception
			this.poscatsAdjadvURL = new URL(poscatsAdjadvURL.toString());
		} catch (MalformedURLException ex) {
			System.err.println("Error: Poscats Adjadv directory URL NOT set");
			ex.printStackTrace();
		}
	}

	public URL getPoscatsNounURL() {
		return poscatsNounURL;
	}

	public void setPoscatsNounURL(URL poscatsNounURL) {
		try {
			// This should not ever throw a malformed url exception
			this.poscatsNounURL = new URL(poscatsNounURL.toString());
		} catch (MalformedURLException ex) {
			System.err.println("Error: Poscats Noun directory URL NOT set");
			ex.printStackTrace();
		}
	}

	public URL getPoscatsOtherURL() {
		return poscatsOtherURL;
	}

	public void setPoscatsOtherURL(URL poscatsOtherURL) {
		try {
			// This should not ever throw a malformed url exception
			this.poscatsOtherURL = new URL(poscatsOtherURL.toString());
		} catch (MalformedURLException ex) {
			System.err.println("Error: Poscats Other directory URL NOT set");
			ex.printStackTrace();
		}
	}

	public URL getPoscatsVerbURL() {
		return poscatsVerbURL;
	}

	public void setPoscatsVerbURL(URL poscatsVerbURL) {
		try {
			// This should not ever throw a malformed url exception
			this.poscatsVerbURL = new URL(poscatsVerbURL.toString());
		} catch (MalformedURLException ex) {
			System.err.println("Error: Poscats Verb directory URL NOT set");
			ex.printStackTrace();
		}
	}
	...
	public MeteorConfiguration(Properties props) {
		...
		// New delta parameters
		String newDelta = props.getProperty("newDeltaWeights");
		if (newDelta != null) {
			ArrayList<Double> newDeltaList = new ArrayList<Double>();
			StringTokenizer ndel = new StringTokenizer(newDelta);
			while (ndel.hasMoreTokens())
				newDeltaList.add(Double.parseDouble(ndel.nextToken()));
			setNewDeltaWeights(newDeltaList);
		}
